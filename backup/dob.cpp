...
void DOB::run()
{
    ...
    double A3[9] = { 0, }, B3[9] = { 0, }, C3[9] = { 0, };
    timer.start();
    for (uint cnt = 0; cnt < 100000; cnt++) {
        for (uint i = 0; i < 3; i++) {
            for (uint j = 0; j < 3; j++) {
                for (uint k = 0; k < 3; k++) {
                    C3[i * 3 + j] += A3[i * 3 + k] * B3[k * 3 + j];
                }
            }
        }
    }

    cout << "Processing Time(Using double precision 1 order array) : " << timer.elapsed() << " ms" << endl;

    double A4[3][3] = { 0, }, B4[3][3] = { 0, }, C4[3][3] = { 0, };
    timer.start();
    for (uint cnt = 0; cnt < 100000; cnt++) {
        for (uint i = 0; i < 3; i++) {
            for (uint j = 0; j < 3; j++) {
    ...
    cout << "Processing Time(Using double precision 2 order array) : " << timer.elapsed() << " ms" << endl;
    ...
    delete[] B5;
    delete[] C5;
    cout << "Processing Time(Using double precision 1 order dynamic array) : " << timer.elapsed() << " ms" << endl;
    ...
    delete[] B6;
    delete[] C6;
    cout << "Processing Time(Using double precision 2 order dynamic array) : " << timer.elapsed() << " ms" << endl;

    Matrix A1 = Matrix(3, 3);
    Matrix B1 = Matrix(3, 3);
    Matrix C1 = Matrix(3, 3);
    ...
    cout << "Processing Time(Using Matrix Class) : " << timer.elapsed() << " ms" << endl;

    vector<double> A2, B2, C2;
    A2.assign(9, 0);
    B2.assign(9, 0);
    C2.assign(9, 0);
    ...
    B2.clear();
    C2.clear();
    cout << "Processing Time(Using std vector) : " << timer.elapsed() << " ms" << endl;
